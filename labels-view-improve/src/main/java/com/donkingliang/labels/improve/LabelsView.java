...
//            }
    ...
    public void setLabels(List<String> labels) {
        setLabels(labels, new LabelTextProvider<String>() {
            @Override
            public CharSequence getLabelText(TextViewImprove label, int position, String data) {
                return data.trim();
            }
        });
    }
    ...
    private <T> void addLabel(T data, int position, LabelTextProvider<T> provider) {
        //更改控件
        final TextViewImprove label = new TextViewImprove(mContext);
    ...
    private void ensureLabelClickable() {
        int count = getChildCount();
        for (int i = 0; i < count; i++) {
            TextViewImprove label = (TextViewImprove) getChildAt(i);
    ...
    public void onClick(View v) {
        if (v instanceof TextViewImprove) {
            TextViewImprove label = (TextViewImprove) v;
            if (!isIndicator) {
    ...
    public void setLabelSelect(TextViewImprove label, boolean isSelect) {
    ...
    private boolean selectChangeIntercept(TextViewImprove label) {
    ...
    private void innerClearAllSelect() {
        int count = getChildCount();
        for (int i = 0; i < count; i++) {
            setLabelSelect((TextViewImprove) getChildAt(i), false);
        }
        mSelectLabels.clear();
    }
    ...
    public void setSelects(int... positions) {
        if (mSelectType != SelectType.NONE) {
            ArrayList<TextViewImprove> selectLabels = new ArrayList<>();
            int count = getChildCount();
            int size = mSelectType == SelectType.SINGLE || mSelectType == SelectType.SINGLE_IRREVOCABLY
                    ? 1 : mMaxSelect;
            for (int p : positions) {
                if (p < count) {
                    TextViewImprove label = (TextViewImprove) getChildAt(p);
    ...
    public interface OnLabelClickListener {
         ...
         * @param position 标签位置
         */
        void onLabelClick(TextViewImprove label, Object data, int position);
    }
    ...
    public interface OnLabelLongClickListener {
         ...
         * @param position 标签位置
         */
        boolean onLabelLongClick(TextViewImprove label, Object data, int position);
    }
    ...
    public interface OnLabelSelectChangeListener {
         ...
         * @param position 标签位置
         */
        void onLabelSelectChange(TextViewImprove label, Object data, boolean isSelect, int position);
    }
    ...
    public interface OnSelectChangeIntercept {
         ...
         * @param position  标签位置
         */
        boolean onIntercept(TextViewImprove label, Object data, boolean oldSelect, boolean newSelect, int position);
    }
    ...
    public interface LabelTextProvider<T> {
         ...
         * @return
         */
        CharSequence getLabelText(TextViewImprove label, int position, T data);
    }
