package com.example.knowledge_android.secondary_list2.adapter;
...
public class RecyclerViewAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> implements View.OnClickListener {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        if (viewType == ViewType.GROUP.getValue()) {
            itemView = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_group_11, parent, false);
            return new GroupViewHolder(itemView);
        } else if (viewType == ViewType.CHILD.getValue()) {
            itemView = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_child_11, parent, false);
    ...
    public void onClick(View v) {
        ...
        if (getItemViewType(position) == ViewType.GROUP.getValue()) {
            ...
            if (Integer.parseInt(v.getTag(R.id.tag_expand_status).toString()) == STATUS_EXPAND) {
                narrow(position);
            } else {
                expand(position);
            }
