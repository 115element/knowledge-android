...
public class PagerSlidingTabStrip extends HorizontalScrollView {
    ...
    public PagerSlidingTabStrip(Context context, AttributeSet attrs, int defStyle) {
        ...
        mTwoLines = a.getBoolean(R.styleable.PagerSlidingTabStrip_pstsTwoLines, false);
    ...
    public void notifyDataSetChanged() {
       Log.i("p","PagerSlidingTabStrip ---> notifyDataSetChanged");
        mTabsContainer.removeAllViews();
        mTabCount = mPager.getAdapter().getCount();
        View tabView;
        for (int i = 0; i < mTabCount; i++) {
            final CharSequence title = mPager.getAdapter().getPageTitle(i);
            final boolean historyTab = title.equals("歷史");
            if (historyTab) {
                tabView = LayoutInflater.from(getContext()).inflate(R.layout.psts_tab_history, this, false);
            } else if (isCustomTabs) {
            ...
            ((TextView) tabView).setSingleLine(false);
            if (mTwoLines) {
                ((TextView) tabView).setSingleLine(false);
                ((TextView) tabView).setLines(2);
            }
    ...
    private void addTab(final int position, CharSequence title, View tabView) {
        final View tabViewContent = tabView.findViewById(R.id.psts_tab_title);
        if (tabViewContent != null && tabViewContent instanceof TextView) {
            TextView textView = (TextView) tabViewContent;
            if (title != null) {
                textView.setText(title);
                textView.setTextColor(Color.RED);
            }
            //PosLog.info("TITLE::::" + title);
            //textView.setTextColor(Color.RED); wuxiao
        }

        //tabView.setBackgroundColor(Color.BLACK); wuxiao
        ...
        tabView.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                for (int i = 0; i < mTabCount; i++) {
                    ...
                    tabView.setBackgroundResource(R.drawable.tab_unpitch_shape);
                }
                v.setBackgroundResource(R.drawable.tab_pitch_shape);
                final View tabViewContent = v.findViewById(R.id.psts_tab_title);
                if (tabViewContent != null) {
                    TextView textView = (TextView) tabViewContent;
         ...
         */
        if (position == 0) {
            tabView.setBackgroundResource(R.drawable.tab_pitch_shape);
            final View tabViewContent1 = tabView.findViewById(R.id.psts_tab_title);
            if (tabViewContent1 instanceof TextView) {
                TextView tab_title = (TextView) tabViewContent1;
    ...
    private void updateTabStyles() {
        ...
        for (int i = 0; i < mTabCount; i++) {
            ...
            final View tabViewContent = v.findViewById(R.id.psts_tab_title);
            if (tabViewContent != null && tabViewContent instanceof TextView) {
                TextView tab_title = (TextView) tabViewContent;
                /**
                 * @author Alex
                 * @since 2021/09/18
                 */
    ...
    private OnGlobalLayoutListener firstTabGlobalLayoutListener = new OnGlobalLayoutListener() {
        ...
        @TargetApi(16) // Build.VERSION_CODES.JELLY_BEAN)
        private void removeGlobalLayoutListenerJB() {
            try {
                final ViewTreeObserver viewTreeObserver = getViewTreeObserver();
                final OnGlobalLayoutListener layoutListener = this;
                ViewTreeObserver.class.getMethod("removeOnGlobalLayoutListener", OnGlobalLayoutListener.class)
                        .invoke(viewTreeObserver, layoutListener);
...
//        }
    ...
    private class PageListener implements OnPageChangeListener {
        ...
        public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
            ...
            int offset = mTabCount > 0 ? (int) (positionOffset * tabView.getWidth()) : 0;
        ...
        public void onPageScrollStateChanged(int state) {
            ...
            currentTab.setBackgroundResource(R.drawable.tab_pitch_shape);
            final View tabViewContent = currentTab.findViewById(R.id.psts_tab_title);
            if (tabViewContent != null) {
                TextView textView = (TextView) tabViewContent;
                textView.setTextColor(Color.WHITE);
            }
    ...
    private void unSelect(View tab) {
        if (tab != null) {
            final View tabViewContent = tab.findViewById(R.id.psts_tab_title);
            if (tabViewContent != null) {
                tabViewContent.setSelected(false);
            }
    ...
    private void select(View tab) {
        if (tab != null) {
            final View tabViewContent = tab.findViewById(R.id.psts_tab_title);
            if (tabViewContent != null) {
                tabViewContent.setSelected(true);
            }
    ...
    public void setTabTextColor(int tabTextColor) {
        ...
        mTabTextColor = createColorStateList(
                tabTextColor,
                tabTextColor,
                Color.argb(DEF_VALUE_TAB_TEXT_ALPHA,
                        Color.red(tabTextColor),
                        Color.green(tabTextColor),
